...
class DrawScopeDirect2D(
    ...
    override fun invalidate(button: Button) {
        invalidButtons += button
        InvalidateRect(hwnd?.reinterpret(),null,1)
    }

    override fun hideButtons(controller: Button) {
        iterateButtons {
            if(it != controller) {
                invalidate(it)
            }
