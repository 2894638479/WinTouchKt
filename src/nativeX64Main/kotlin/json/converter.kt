...
fun ButtonJson.toButton(
    style: ButtonStyleJson,
    stylePressed: ButtonStyleJson,
    offset: Point,
    scale:Float,
    outlineWidth:Float
): Button {
    ...
    val outlineWidth = this.outlineWidth ?: outlineWidth
    val shape = rect?.toRect(outlineWidth)
        ?: roundedRect?.toRoundedRect(outlineWidth)
        ?: round?.toRound(outlineWidth)
        ?: nullPtrError()
    return Button(
        name,
        key,
...
fun GroupJson.toGroup(
    ...
    val btns = buttons.map { it.toButton(
        style,
        stylePressed,
        offset.toPoint(),
        scale,
